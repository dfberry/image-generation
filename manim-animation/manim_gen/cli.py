...
def generate_video(
    ...
    llm_output = client.generate_scene_code(prompt, duration, model=model)
    
    # Step 2: Build and validate scene
    with tempfile.TemporaryDirectory() as tmpdir:
        scene_file = Path(tmpdir) / "scene.py"
        logger.info("Building and validating scene")
        code, scene_path = build_scene(llm_output, scene_file)
        ...
        video_path = render_scene(scene_path, output, quality)
