...
from remotion_gen.errors import RemotionGenError, LLMError, RenderError, ValidationError
...
def generate_video(
    ...
    print(f"Generating {duration}s video at {preset.resolution_name} {preset.fps}fps...")
    ...
    try:
        component_path = write_component(component_code, project_root, debug)
        if debug:
...
Examples:
  ...
  remotion-gen --prompt "Text fading in and out" --quality high --duration 10 --output fade.mp4
...
Environment variables:
  ...
  AZURE_OPENAI_DEPLOYMENT     Azure OpenAI deployment name
    ...
    parser.add_argument(
        ...
        help=f"Video duration in seconds ({MIN_DURATION_SECONDS}-{MAX_DURATION_SECONDS})",
    )
